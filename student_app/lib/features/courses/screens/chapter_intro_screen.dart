...
class ChapterIntroScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final savedIntroduction = introduction?.trim();
    final hasIntroduction = savedIntroduction?.isNotEmpty == true;
    final normalizedTitle = chapterTitle.toLowerCase().replaceAll('é', 'e');
    final isDerivativeChapter =
        subjectName.toLowerCase().contains('math') &&
        RegExp(
          r'(^|[^a-z])(derivation|derivee|derivees)([^a-z]|$)',
        ).hasMatch(normalizedTitle);
    return Scaffold(
      backgroundColor: const Color(
        0xFF0A1128,
      ), // Bleu nuit immersif de la maquette
      appBar: AppBar(
        ...
        title: Text(
          gradeName?.trim().isNotEmpty == true
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              else if (!isDerivativeChapter)
                const Text(
                  'L’introduction de ce chapitre n’est pas encore renseignée. Vous pouvez consulter les leçons disponibles ci-dessous.',
                  style: TextStyle(
              ...
              if (!hasIntroduction && isDerivativeChapter) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(AppRadius.card),
                    border: Border.all(
                      color: const Color(0xFF9333EA).withAlpha(100),
                    ),
                  ),
                  child: Row(
                    children: [
