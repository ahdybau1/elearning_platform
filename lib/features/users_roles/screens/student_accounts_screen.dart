...
class StudentAccountsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<StudentAccountsScreen> createState() =>
      _StudentAccountsScreenState();
}
...
class _StudentAccountsScreenState extends ConsumerState<StudentAccountsScreen> {
  ...
  Widget build(BuildContext context) {
    final accountsAsync = ref.watch(
      accountsProvider(_search.isEmpty ? null : _search),
    );
          ...
          Expanded(
            child: accountsAsync.when(
              data: (accounts) {
                ...
                // Sur mobile, une vraie liste de cartes plutôt qu'un DataTable qu'on ferait défiler
                // horizontalement — retour utilisateur réel (2026-08-30) : un tableau à faire
                // défiler reste une mauvaise expérience mobile, certains éléments doivent
                // s'adapter/se réorganiser, pas juste devenir scrollables dans les deux sens.
                if (MediaQuery.of(context).size.width < 700) {
                  return ListView.builder(
                    itemCount: accounts.length,
                    itemBuilder: (context, idx) => _buildMobileAccountCard(context, accounts[idx]),
                  );
                }
                return Container(
                  ...
                  child: ClipRRect(
                    borderRadius: BorderRadius.circular(16),
                    // Défilement horizontal ajouté (2026-08-30, retour utilisateur réel) : sans lui,
                    // un DataTable plus large que l'écran était simplement ROGNÉ par le ClipRRect —
                    // des colonnes entières (ex: numéro de téléphone) devenaient illisibles sur
                    // mobile, pas juste "serré". Le SingleChildScrollView vertical existant reste à
                    // l'intérieur (défilement des lignes), celui-ci gère la largeur.
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      IconButton(
                                        ...
                                        visualDensity: VisualDensity.compact,
                                        tooltip:
                                            'Gérer les sessions (déconnexion forcée)',
  ...
  /// Équivalent mobile d'une ligne de DataTable — même information (identité, contact, date,
  /// actions), réorganisée verticalement au lieu de colonnes fixes. Les 3 IconButton d'actions
  /// restent identiques (mêmes tooltips/callbacks), juste disposés en Wrap plutôt qu'en Row pour ne
  /// jamais déborder même sur un très petit écran.
  Widget _buildMobileAccountCard(BuildContext context, Account account) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              Text(
                account.phone ?? 'Téléphone non renseigné',
                style: GoogleFonts.inter(fontSize: 12, color: Colors.white70),
              ),
              Text(
                'Créé le ${account.createdAt.toLocal().toString().split(' ').first}',
          ...
          Wrap(
            ...
            children: [
              ...
              IconButton(
                icon: const Icon(Icons.phonelink_erase_rounded, size: 18, color: AppTheme.accentAmber),
        ...
        builder: (ctx, setModalState) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Email (non modifiable) : ${account.email}',
                  style: GoogleFonts.inter(
                    fontSize: 12,
                    color: AppTheme.textMuted,
                  ),
          ...
          child: Consumer(
            builder: (context, ref, _) {
              final profilesAsync = ref.watch(
                profilesForAccountProvider(account.id),
              );
              return profilesAsync.when(
                data: (profiles) {
                  final allActive =
                      profiles.isNotEmpty &&
                      profiles.every((p) => p.status == 'actif');
                  final allArchived =
                      profiles.isNotEmpty &&
                      profiles.every((p) => p.status == 'archive');
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      if (allActive || allArchived) ...[
                        Align(
                          ...
                          child: TextButton.icon(
                            ...
                            label: Text(
                              allActive
                                  ? 'Archiver tous les profils'
                                  : 'Désarchiver tous les profils',
                              style: GoogleFonts.inter(
                                fontSize: 12,
                                color: AppTheme.accentAmber,
                              ),
                      ...
                      Expanded(
                        child: profiles.isEmpty
                            ? Center(
                                child: Text(
                                  'Aucun profil — utilisez "Ajouter un profil" ci-dessous.',
                                  style: GoogleFonts.inter(
                                    color: AppTheme.textMuted,
                                  ),
                                ),
                      ...
                      SizedBox(
                        ...
                        child: OutlinedButton.icon(
                          onPressed: () =>
                              _showAddProfileDialog(context, account),
                          icon: const Icon(Icons.add_rounded, size: 18),
                          label: const Text(
                            'Ajouter un profil (ré-inscription)',
                          ),
                        ),
                      ),
                    ],
  ...
  Widget _buildProfileTile(
    ...
    return ListTile(
      ...
      subtitle: Text(
        'Palier: ${p.subscriptionTier} · ${p.status}',
      ...
      trailing: IconButton(
        ...
        onPressed: () async {
          ...
          try {
            final service = ref.read(supabaseServiceProvider);
            await service.setProfileStatus(
              p.id,
              isActive ? 'archive' : 'actif',
            );
            ...
            messenger.showSnackBar(
              SnackBar(
                backgroundColor: AppTheme.accentEmerald,
                content: Text(
                  isActive ? 'Profil archivé.' : 'Profil désarchivé.',
                ),
              ),
            );
          } catch (e) {
              ...
              onPressed: isLoading
                  ...
                  : () async {
                      if (selectedClassId == null) {
                        setModalState(
                          () => formError = 'Sélectionnez une classe.',
                        );
                        return;
                      }
                      if (schoolYearCtrl.text.trim().isEmpty) {
                        setModalState(
                          () =>
                              formError = 'L\'année scolaire est obligatoire.',
                        );
                        return;
                      }
                      setModalState(() {
            ...
            child: Consumer(
              builder: (context, ref, _) {
                final sessionsAsync = ref.watch(
                  sessionsForAccountProvider(account.id),
                );
              ...
              onPressed: isLoading
                  ...
                  : () async {
                      ...
                      if (emailCtrl.text.trim().isEmpty) {
                        setModalState(
                          () => formError = 'L\'email est obligatoire.',
                        );
                        return;
                      }
