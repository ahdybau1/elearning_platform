...
class _DonationsScreenState extends ConsumerState<DonationsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final donationsAsync = ref.watch(
      donationsProvider(_donationCampaignFilter),
    );
  ...
  Widget _buildCampaignsTab(AsyncValue<List<CharityCampaign>> campaignsAsync) {
    return campaignsAsync.when(
      ...
      data: (campaigns) {
        ...
        // crossAxisCount fixé à 2 sans seuil mobile : chaque carte de campagne n'avait plus
        // qu'environ 170px de large sur téléphone (retour utilisateur réel, 2026-09-02).
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Text(
              type == 'video'
                  ? 'Voir la vidéo illustrative'
                  : 'Écouter l\'illustration audio',
              style: GoogleFonts.inter(
                fontSize: 12,
                color: AppTheme.accentCyan,
              ),
  ...
  Widget _buildCampaignCard(CharityCampaign c) {
    final progress =
        (c.collectedAmount / (c.targetAmount > 0 ? c.targetAmount : 1)).clamp(
          0.0,
          1.0,
        );
    ...
    return Opacity(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: c.isActive ? AppTheme.borderColor : AppTheme.accentAmber,
          ),
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
              ...
              children: [
                ...
                if (!c.isActive)
                  IconButton(
                    ...
                    onPressed: () =>
                        _showDeleteCampaignConfirmation(context, c),
                  ),
              ],
            ),
          ],
    ...
    try {
      ...
      messenger.showSnackBar(
        SnackBar(
          backgroundColor: AppTheme.accentEmerald,
          content: Text(
            isActive ? 'Campagne désarchivée.' : 'Campagne archivée.',
          ),
        ),
      );
    } catch (e) {
  ...
  void _showArchiveCampaignConfirmation(
  ...
  void _showDeleteCampaignConfirmation(
    BuildContext context,
    CharityCampaign c,
  ) {
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  onChanged: (v) =>
                      setModalState(() => nameMatches = v.trim() == c.title),
                ),
                if (errorText != null) ...[
                  const SizedBox(height: 12),
                  Text(
                      ...
                      try {
                        ...
                        messenger.showSnackBar(
                          SnackBar(
                            backgroundColor: AppTheme.accentRose,
                            content: Text(
                              'Campagne "${c.title}" supprimée définitivement.',
                            ),
                          ),
                        );
                      } catch (e) {
                ...
                items: [
                  ...
                  ...(campaignsAsync.valueOrNull ?? []).map(
                    (c) => DropdownMenuItem<String?>(
                      value: c.id,
                      child: Text(
                        c.isActive ? c.title : '${c.title} — archivée',
                        overflow: TextOverflow.ellipsis,
                      ),
                    ),
                  ),
  ...
  void _showCampaignDialog(BuildContext context, {CharityCampaign? existing}) {
    ...
    final targetCtrl = TextEditingController(
      text: existing?.targetAmount.toStringAsFixed(0) ?? '1000000',
    );
    String? mediaUrl = existing?.imageUrl;
    String? mediaFilename = existing?.imageUrl != null
        ? existing!.imageUrl!.split('/').last
        : null;
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Illustration (image, audio ou vidéo — optionnel)',
                    style: GoogleFonts.inter(
                      fontSize: 12,
                      fontWeight: FontWeight.w600,
                      color: AppTheme.textSecondary,
                      ...
                      children: [
                        OutlinedButton.icon(
                          onPressed: isUploadingMedia
                              ? null
                              : () => pickAndUploadMedia(setDlgState, ref),
                        ...
                        if (mediaFilename != null)
                          Expanded(
                            child: Row(
                              children: [
                                Icon(
                                  _iconForCampaignMedia(mediaFilename!),
                                  size: 14,
                                  color: AppTheme.accentCyan,
                                ),
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: isLoading
                  ...
                  : () async {
                      ...
                      if (description.isEmpty) {
                        setDlgState(
                          () => submitError = 'La description est obligatoire.',
                        );
                        return;
                      }
                      ...
                      if (target == null || target <= 0) {
                        setDlgState(
                          () => submitError =
                              'Objectif invalide — entrez un nombre positif.',
                        );
                        return;
                      }
                      setDlgState(() {
              ...
              child: isLoading
                  ...
                  : Text(
                      isEditing ? 'Enregistrer' : 'Lancer la Campagne',
                      style: const TextStyle(color: Colors.white),
                    ),
            ),
          ],
                      ...
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          items: _donationOperators
                              ...
                              .toList(),
                          onChanged: (v) =>
                              setDlgState(() => operator = v ?? operator),
                        ),
                      ),
                    ],
                  ),
                  ...
                  : () async {
                      ...
                      try {
                        ...
                        await service.createDonation(
                          donorName: name,
                          donorEmail: emailCtrl.text.trim().isEmpty
                              ? null
                              : emailCtrl.text.trim(),
                          donorPhone: phoneCtrl.text.trim().isEmpty
                              ? null
                              : phoneCtrl.text.trim(),
                          amount: amount,
                          operator: operator,
