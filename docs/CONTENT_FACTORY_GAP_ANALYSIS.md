...
| Model Router — abstraction par capability (§5) [IA-005, FAIT 2026-08-29] | X | | | `gateway/app/model_router/`, `supabase/functions/ai-generate-text` | `ai_agents`/`ai_agent_versions` (migration 61) | Aucune — vérifié réellement (pedagogy_small : Gemini, réponse correcte). **Écart réel documenté** : `ANTHROPIC_API_KEY` absente des secrets du projet — `reasoning_strong` retombe toujours sur Gemini aujourd'hui, comme les 3 Edge Functions de génération existantes qui ont le même repli silencieux | Faible — fonctionnel, mais préférence Claude non exercée en pratique tant que la clé n'est pas fournie |
