...
| Agent Orchestrator (LangGraph) / modèles auto-hébergés (vLLM/Ollama) / Compute Scheduler | | | X | aucun | aucune | **C'est la partie qui exige réellement un nouvel hébergement** (service Python persistant, potentiellement GPU) — U6 du cahier autorise « machine développeur » comme premier nœud Compute Fabric légitime à coût nul, mais ça reste une vraie décision à prendre explicitement avec l'utilisateur avant de commencer | Élevé si démarré sans décision d'hébergement explicite |
