...
| **Profil** | Profil élève & gestion de compte | `StudentProfileScreen` | **IMPROVE** (découpage fait le 17/09/2026, voir `DEVELOPMENT_PROGRESS.md` lot 12) | Édition nom, avatar, photo upload, link parent | `accounts`, storage `avatars`, `parent_profile_links` | Réelles | ~~Page très longue (1074 lignes) monolithique~~ Découpée en 12 sous-composants (`lib/features/profile/widgets/`), orchestrateur ~65 lignes | Reste : mettre en valeur les statistiques et badges (changement visuel, hors périmètre du découpage structurel) |
